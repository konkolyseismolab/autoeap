...
def detrend_wrt_PDM(targettpf,time,flux,fluxerr,polyorder=9,sigma=10,show_plots=False,save_plots=False,debug=False):
    ...
    ax1.plot(x,y,'.')
    ax1.plot( x,  np.poly1d(np.polyfit(x,y,polyorder))(x) ,'k')

    ax1.plot( x, np.poly1d(np.append(res.x,const))(x) ,'r' )
    ax1.grid()

    ax2.set_title('Corrected light curve')
    ax2.plot( x, y-np.poly1d(np.append(res.x,const))(x) ,'C0.'  )
    ax2.grid()

    ax3.set_title('Original')
    ax3.scatter( (x*testf)%1, y, c=x, s=4,cmap='jet')
    ax3.grid()
    ax4.set_title('Corrected')
    ax4.scatter( (x*testf)%1, y-np.poly1d(np.append(res.x,const))(x), c=x, s=4,cmap='jet')
    ax4.grid()
