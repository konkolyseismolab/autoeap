...
def detrend(dataset,campaign=5,splits=None,quiet=False,save_dir='.',seed=0,flux_type='sap',default_position_kernel='SqrExp',
    ...
    default_splits = {0:[1938.1],1:[2021],2:[2102],3:[2154,2190], 4:[2240,2273], 5:[2344], 6:[2390,2428], 7:[2468.5,2515], 8:[2578.51,2598.24], 92:[2727.51],
                      102:[2778], 112:[2862.25], 12:[2915,2951], 13:[2998,3033],14:[3085,3123.75],15:[3170,3207.5],16:[3297.5,3331],17:[3367,3400],18:[3425,3460],19:[]}
    ...
    for iset in range(ds.nsets):
        ...
        cflux = flux-ptrend+np.median(ptrend)-ttrend+np.median(ttrend)
    ...
    for iset in range(ds.nsets):
        ...
        if np.isfinite(ds.fluxes[iset]).sum() >= 100:
            ...
            ## Trend computation
            ## -----------------
            (mt,tt),(mp,tp) = map(lambda a: (np.nanmedian(a), a-np.nanmedian(a)), detrender.predict(pv, components=True))
